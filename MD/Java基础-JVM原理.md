...
具体内容见![](../images/ConstantsPool.jpg)
* 常量池是.class文件里面的表，记录了执行的类名、方法名、参数类型、字面量信息等信息。
* 字符串常量池(StringTable),1.6之前在方法区，1.7以及以后在堆内存中。
* 永久代是 JDK 1.8 之前的方法区实现，JDK 1.8 及以后方法区的实现便成为元空间。永久代有一个 JVM 本身设置的固定大小上限,元空间使用的是直接内存，受本机可用内存的限制。
...
3.	pc寄存器(程序计数器): 任意时刻线程只会执行一个方法的代码，如果不是native的，就存放当前正在执行的字节码指令的地址，如果是native，则是undefined
...
* 线程内读到的结果是是按顺序执行的结果，但是执行顺序不一定是按照顺序的，多线程的时候会出现问题。
2. 锁定规则：一个unLock操作先行发生于后面对同一个锁额lock操作；
* synchronized使用对象的Mointor实现，在进入代码块前执行MointorEnter，退出时时执行MointorExit。MointorEnter和MointorExit也会防止重排序。
3. volatile变量规则：对一个变量的写操作先行发生于后面对这个变量的读操作；
* volatile确保单次数据操作（i++不是单次数据操作）是原子性的。
* volatile变量写入的时候，会立即同步变量缓存到主内存中，同时使得其它cpu缓存结果失效。
* 一个volatile典型例子是AtomicLong的updateAndGet函数防止了读写的重排序。
